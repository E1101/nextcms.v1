...
define([
	...
	"core/js/views/UserItemView"
], function(dojoDeclare, dojoDomConstruct, dijitRegistry) {
	return dojoDeclare("core.js.views.UserListView", null, {
		// _id: String
		_id: null,
		...
		_init: function() {
			...
			dojo.query(".coreUserItem", this._id).forEach(function(node, index, arr) {
				var userItemView = new core.js.views.UserItemView(node, that);
				that._userItemMap[userItemView.getUser().user_id + ""] = userItemView;
			});
		...
		removeUserItemView: function(/*String*/ userId) {
			...
			if (userItemView) {
				...
				dojoDomConstruct.destroy(userItemView.getDomNode());
