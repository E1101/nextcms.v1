...
define([
	...
	// DOJO LESSON: Require this to allow dragging and dropping external DnD source to a grid container
	// HACK: In Dojo 1.8.0, I have to edit the dojox.mdnd.adapter.DndFromDojo class
	//	constructor: function() {
	//		// this._dojoManager = dojo.dnd.manager();
	//		this._dojoManager = dojo.dnd.Manager.manager();
	//	},
...
], function(dojoArray, dojoDeclare, dojoLang, dojoAspect, dojoDom, dojoDomAttr, dojoDomClass, dojoDomConstruct, dojoDomStyle, dojoIoQuery, dojoJson, dojoOn, dojoRequestXhr, dojoTopic, dijitRegistry) {
	return dojoDeclare("core.js.views.LayoutContainer", null, {
			...
			for (var region in regions) {
				...
				var dndSource = new dojo.dnd.Source(childContainer.domNode, {
					...
					onDropExternal: function(source, nodes, copy) {
						...
						that.addBorderContainers(dijitRegistry.byNode(this.node), regions);
					}
				});
		...
		deleteBorderContainer: function(/*dijit.layout.BorderContainer*/ container) {
			...
			var parent = dijitRegistry.byNode(container.domNode.parentNode); 
			if (parent) {
			...
			if (parent.getChildren().length == 0) {
				dojoDomClass.add(parent.domNode, "app-container-border");
			}
		...
		addGridContainer: function(/*dijit.layout.BorderContainer|dijit.layout.TabContainer*/ container, /*Object?*/ settings) {
			...
			settings = dojoLang.mixin({
				...
				"class": "module-core-layout-grid"
			}, settings);
			var gridContainer = new dojox.layout.GridContainer(settings);
			container.addChild(gridContainer);
			container.resize();
			
			// HACK: In Dojo 1.8.0, I have to set the height for grid container
			dojoDomStyle.set(gridContainer.gridContainerDiv, "height", "100%");
		...
		addTabContainer: function(/*dijit.layout.BorderContainer|dojox.layout.GridContainer*/ container, /*Integer?*/ zoneIndex, /*Integer?*/ portletIndex) {
			...
			switch (true) {
				...
				case (container instanceof dojox.layout.GridContainer):
					...
					dojoAspect.after(container, "setColumns", function(numColumns) {
		...
		addMainContentPane: function(/*dijit.layout.BorderContainer|dojox.layout.GridContainer*/ container, /*Integer?*/ zoneIndex, /*Integer?*/ portletIndex) {
			...
			var pane = new dijit.layout.ContentPane({
				...
				content: '<div class="app-center"><div>' + this._i18n.page._share.mainContent + "</div></div>"
			});
			switch (true) {
				case (container instanceof dijit.layout.BorderContainer):
			...
			dojoDomClass.add(pane.domNode, ["app-container-border", "appMainContentPane", core.js.LayoutConstant.PORTLET_CONTAINER_CLASS]);
		...
		getMainContentPane: function() {
			...
			return (nodes.length == 0) ? null : dijitRegistry.byNode(nodes[0]); // dijit.layout.ContentPane
		},
		...
		addPortlet: function(/*dojox.layout.GridContainer|DomNode*/ container, /*Object*/ widget, /*Integer*/ zoneIndex, /*Integer*/ portletIndex, /*Boolean*/ showSettings) {
			...
			var that = this;
			dojoAspect.after(portletSettings, "toggle", function() {
				...
				if ("block" == display) {
					...
					//		});
					dojoRequestXhr(url, {
						data: dojoLang.mixin(params, widget.params),
						method: "POST"
					}).then(function(data) {
		...
		_activateTabInsidePortlet: function(/*dojox.layout.ContentPane*/ pane) {
			...
			dojo.query(".dijitTabContainer", pane.domNode).forEach(function(node) {
				var tabContainer = dijitRegistry.byNode(node);
				if (tabContainer) {
			...
			switch (true) {
				...
				case (container instanceof HTMLElement):
					dojo.query(".dojoxPortlet", container).forEach(function(portletNode) {
						var portlet = dijitRegistry.byNode(portletNode);
...
//				tinyMCE.remove(editor);
		...
		switchToEditMode: function() {
			...
			dojoDomClass.remove(this._id, "app-content-pane");
			...
			if (this._layoutData.properties) {
				this.setProperties(dijitRegistry.byId(this._id), this._layoutData.properties);
			}
			
			for (var i in this._layoutData.containers) {
				this._toEditMode(dijitRegistry.byId(this._id), this._layoutData.containers[i]);	
			}
		},
				...
				onDropExternal: function(source, nodes, copy) {
					...
					switch (type) {
						case "layout":
							...
							that.addBorderContainers(dijitRegistry.byId(that._id), data);
							break;
						case "widget":
							break;
						default:
		...
		_enableWidgetDnd: function() {
			...
			core.js.base.controllers.Subscriber.subscribe(this.TOPIC_GROUP, "/dojox/mdnd/drag/start", this, function(node, sourceArea, sourceDropIndex) {
				...
				var portlet = dijitRegistry.byNode(node);
				if (!portlet) {
					return;
				}
		...
		_getPortletIndices: function(/*core.js.views.LayoutPortlet*/ portlet) {
			...
			var gridContainer = dijitRegistry.byNode(gridNode);
			...
			return {
				grid: dijitRegistry.byNode(gridNode),
				zoneIndex: zoneIndex,
				portletIndex: portletIndex
			};		// Object
		},
		...
		switchToViewMode: function() {
			...
			dojoDom.byId(this._id).innerHTML = "";
			dojoDomClass.remove(this._id, "app-container-border");
			...
			dijitRegistry.byId(this._id).addChild(contentPane);
		...
		_disableLayoutDnd: function() {
			...
			this._disableDnd(dijitRegistry.byId(this._id));
		},
		...
		getHtmlData: function() {
			...
			//		for searching later.
			return this._getHtmlData(dijitRegistry.byId(this._id));		// String
		},
		...
		getLayoutData: function() {
			...
			return this._getContainerData(dijitRegistry.byId(this._id));	// Object
		},
		...
		_getWidgetParams: function(/*core.js.views.LayoutPortlet*/ portlet) {
			...
			dojo.query("." + core.js.LayoutConstant.WIDGET_INPUT_CLASS, portlet.domNode).forEach(function(inputNode) {
				var widget = dijitRegistry.byNode(inputNode), name = null, value;
				if (widget) {
					switch (true) {
						...
						case (widget instanceof dijit.form.RadioButton):
							...
							break;
						case (widget instanceof dijit._WidgetBase):
						default:
							name  = widget.get("appWidgetInputName");
					...
					switch (true) {
						case ("TEXTAREA" == inputNode.tagName):
							name  = dojoDomAttr.get(inputNode, "appWidgetInputName");
							value = dojoDomClass.contains(inputNode, core.js.LayoutConstant.WIDGET_INPUT_TINYMCE_CLASS)
								  ? tinyMCE.getInstanceById(dojoDomAttr.get(inputNode, "id")).getContent()
								  : inputNode.innerHTML;
							break;
						case ("hidden" == dojoDomAttr.get(inputNode, "type")):
							// Hidden element
		...
		_loadWidgetParams: function(/*dojox.layout.ContentPane*/ pane, /*Object*/ widget) {
			...
			dojo.query("." + core.js.LayoutConstant.WIDGET_INPUT_CLASS, pane.domNode).forEach(function(node) {
				var dijitWidget = dijitRegistry.byNode(node);
					...
					if (name && widget.params && widget.params[name]) {
						...
						switch (true) {
							case ("TEXTAREA" == node.tagName):
								...
								break;
							case ("hidden" == dojoDomAttr.get(node, "type")):
