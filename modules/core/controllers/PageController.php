	...
	public function importAction()
	{
		...
		switch ($format) {
			case 'json':
				...
				// Return the format required by Dojo uploader
				if ('html5' == $request->getParam('uploader')) {
					// HTML5 uploader
					$this->_helper->json($uploadedFile);
				} else {
					// Flash uploader
					...
					$return	= array(
						'name=' . (isset($uploadedFile['name'])		 ? $uploadedFile['name']	  : ''),
						// 'type=' . (isset($uploadedFile['extension']) ? $uploadedFile['extension'] : ''),
						'size=' . (isset($uploadedFile['size'])		 ? $uploadedFile['size']	  : 0),
						'appUploadedFiles=' . rawurlencode(base64_encode(Zend_Json::encode($uploadedFile))),
					);
		...
		switch ($format) {
			case 'json':
				...
				$data = array(
					...
					'paginator' => $this->view->paginator('slidingToolbar')->render($paginator, "javascript: require(['dojo/topic'], function(dojoTopic) { dojoTopic.publish('" . $paginatorTopic . "', __PAGE__); });"),
				);
